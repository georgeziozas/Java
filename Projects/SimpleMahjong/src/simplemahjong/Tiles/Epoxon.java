...
/**
 * @desc Epoxon class extending Tiles class.
 */

public class Epoxon extends Tiles
{
     ...
     * @param colour             , a the colour of Epoxon which will be white by default(my convention)
     */

    public Epoxon(String specificTypeOfTile, int[] position)
    {
     ...
     * @desc Gives the 'Epoxon' value to this tiles.generalTypeTile.
     */
    public generalTypeTile getGeneralTypeTile() {
        return simplemahjong.Game.generalTypeTile.EPOXON;
    }
    ...
    public boolean isValidTiles(Tiles end)
    {
        ...
        for (List l : Board.board)
        {
            if (getGeneralTypeTile().equals(end.getGeneralTypeTile()))   /* Checks if they are in the same category */
                validFlag = true;
            else if (end.getGeneralTypeTile().equals(simplemahjong.Game.generalTypeTile.LOYLOYDION))  /* Checks if the other Tile is in the Louloudion Category, cause in the game epoxes and louloudia can be combined. */
                validFlag = true;
